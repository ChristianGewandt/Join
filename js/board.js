...
function generateAvatarHtml(assignedTo) {
  ...
  if (assignedTo.length === 3) {
    ...
    avatarHtml += `<div class="boardAvatar" style="background-color:black"><span>+${
      assignedTo.length - 2
    }</span></div>`;
  } else {
    for (let index = 0; index < assignedTo.length; index++) {
...
function loadTaskPrio(i) {
  if (tasks[i].prio.name === "Low") {
    ...
    document
      .getElementById("prio-medium-white-icon2")
    ...
    document
      .getElementById("prio-urgent-white-icon2")
      .classList.remove("d-none");
